...
function validBarcode(value: string): string {
  ...
  return /^(?:\d{8}|\d{12}|\d{13})$/.test(normalized) ? normalized : "";
}
...
export function BrowserBarcodeScannerPanel() {
  ...
  const lookup = React.useCallback(async (raw: string) => {
    ...
    if (!code) {
      toast.error("Geçerli bir EAN / UPC barkodu girin.");
      return;
    }
      ...
      <Card>
        <CardContent className="space-y-4 p-5">
          ...
          <div className="flex gap-2">
            <input
              value={barcode}
              onChange={(event) => setBarcode(event.target.value.replace(/\D/g, "").slice(0, 13))}
              onKeyDown={(event) => {
        ...
        <Card>
          <CardContent className="flex gap-3 p-5 text-sm">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Diewish eksik besin değerlerini AI ile uydurmaz.</p>
            </div>
          </CardContent>
        </Card>
      )}
