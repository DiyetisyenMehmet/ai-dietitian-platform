...
const envSchema = z.object({
  ...
  ABACUS_API_ENDPOINT_URL: z.string().url().default("https://api.abacus.ai/api/v0/getApiEndpoint"),
  ...
  // Google Play Billing. The service-account private key is a secret and must
  // come from the runtime secret store, never from source control or the app.
  ...
  GOOGLE_PLAY_PREMIUM_PRODUCT_ID: z.string().default("diewish_premium"),
  GOOGLE_PLAY_PREMIUM_PLUS_PRODUCT_ID: z.string().default("diewish_premium_plus"),
...
export const corsOrigins: string[] = env.CORS_ORIGIN.split(",").map((origin) => origin.trim()).filter(Boolean);