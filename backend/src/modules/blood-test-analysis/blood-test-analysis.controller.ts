...
/** Processing older than this cannot be a healthy synchronous request anymore. */
...
/**
 * Repairs records left PROCESSING when a request/container was interrupted so
 * the client never has to display an eternal "Analiz ediliyor" state. This is
 * deterministic DB reconciliation only; it never retries or spends AI credits.
 */
async function reconcileStaleProcessing(userId: string): Promise<void> {
  const cutoff = new Date(Date.now() - STALE_PROCESSING_MS);
  await bloodTestAnalysisRepository.failStaleProcessingForUser(userId, cutoff);
}
