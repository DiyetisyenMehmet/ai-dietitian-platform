...
/**
 * Cheap deterministic quality gate. It intentionally rejects only cases we can
 * prove are unusable (tiny/blank/near-solid images); semantic food-vs-object
 * classification is left to the vision model.
 */
async function assertUsableImage(buffer: Buffer): Promise<Buffer> {
  ...
  // Downscale first: this makes both the deterministic stats check and the
  // external vision request cheaper while preserving enough visual detail.
...
export const foodScanService = {
  async analyze(buffer: Buffer): Promise<FoodScanResult> {
    ...
    if (!result.isFood || result.confidence < FOOD_IMAGE_MIN_CONFIDENCE || result.items.length === 0) {
