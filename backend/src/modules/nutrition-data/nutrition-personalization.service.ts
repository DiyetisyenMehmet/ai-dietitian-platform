...
function sum(values: Array<number | null>): number { return Math.round(values.reduce((total, value) => total + (value ?? 0), 0) * 10) / 10; }
...
function allergenMatch(food: CanonicalFood, allergies: string[]): string[] {
  ...
  return allergies.filter((allergy) => { const key = normalize(allergy); return declared.some((item) => item.includes(key) || key.includes(item)); });
}
...
function nutrientDifference(source: NutrientValues, alternative: NutrientValues): Partial<Record<keyof NutrientValues, number>> {
  ...
  for (const key of keys) { const a = source[key]; const b = alternative[key]; if (a !== null && b !== null) result[key] = Math.round((b - a) * 10) / 10; }
  return result;
}
...
async function loadLocalContext(userId: string) {
  ...
  return {
    profile, plan,
    consumed: { calories: sum(meals.map((m) => m.calories)), proteinG: sum(meals.map((m) => m.proteinG)), carbohydratesG: sum(meals.map((m) => m.carbsG)), fatG: sum(meals.map((m) => m.fatG)) },
  };
}
...
async function personalizeLocal(userId: string, nutrients: NutrientValues, food?: CanonicalFood) {
  ...
  const metrics = plan ? deriveNutritionPersonalization(nutrients, {
    calories: plan.dailyCalories, proteinG: plan.proteinGrams, carbohydratesG: plan.carbsGrams, fatG: plan.fatGrams, mealsPerDay: plan.mealsPerDay,
  }, consumed) : null;
  ...
  return { metrics, profileContextUsed: Boolean(profile), activePlanUsed: Boolean(plan), dietaryCompatibility: dietary, allergenDataComplete: food ? food.allergens.length > 0 : false, warnings, windowHours: 24 };
}
...
function validateNutrients(input: NutrientValues): NutrientValues {
  ...
  for (const key of keys) {
    ...
    if (value !== null && (!Number.isFinite(value) || value < 0 || value > 100_000)) throw ApiError.badRequest(`Geçersiz nutrient değeri: ${key}`);
  }
  return input;
}
...
export const nutritionPersonalizationService = {
  ...
  async personalizeNutrients(userId: string, nutrients: NutrientValues) {
    const verified = validateNutrients(nutrients);
    return { nutrients: verified, ...(await personalizeLocal(userId, verified)) };
  },

  async compareBarcode(userId: string, barcode: string, grams: number, requestedQueries?: string[]) {
    validGrams(grams);
    const source = await nutritionDataService.getByBarcode(barcode);
    ...
    const queries = (requestedQueries?.length ? requestedQueries : [...DEFAULT_ALTERNATIVES]).map((q) => q.trim()).filter((q) => q.length >= 2).slice(0, 8);
    ...
    for (const query of queries) {
      ...
      } catch { /* candidate failure is isolated */ }
    }
    ...
    const comparisons = candidates
      ...
      .map((food) => {
        ...
        return { food: { externalId: food.externalId, provider: food.provider, displayNameTr: food.displayNameTr }, servingGrams: comparison.servingGrams, targetCalories: comparison.targetCalories, nutrients: comparison.nutrients,
          differenceFromSource: nutrientDifference(sourcePortion.nutrients, comparison.nutrients), dietaryCompatibility: dietaryCompatibility(food, profile?.dietaryPreference ?? null), allergenDataComplete: food.allergens.length > 0 };
      })
      .filter((value): value is NonNullable<typeof value> => value !== null).slice(0, 5);

    return { source: { food: { displayNameTr: source.displayNameTr, provider: source.provider }, grams: sourcePortion.grams, nutrients: sourcePortion.nutrients }, comparisons,
      warning: "Aynı kalori, besinsel eşdeğerlik anlamına gelmez. Protein, karbonhidrat, yağ, lif ve şeker farklarını birlikte değerlendir." };
  },
};
