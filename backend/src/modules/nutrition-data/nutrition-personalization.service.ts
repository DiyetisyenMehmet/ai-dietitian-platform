...
function sum(values: Array<number | null>): number {
  const total = values.reduce<number>((running, value) => running + (value ?? 0), 0);
...
function dietaryCompatibility(
  ...
  if (preference === "VEGAN") {
    return food.vegan === true ? "COMPATIBLE" : food.vegan === false ? "INCOMPATIBLE" : "UNKNOWN";
  }
  if (preference === "VEGETARIAN") {
    return food.vegetarian === true
      ? "COMPATIBLE"
  ...
  if (preference === "GLUTEN_FREE") {
    return food.glutenFree === true
      ? "COMPATIBLE"
      : food.glutenFree === false
        ? "INCOMPATIBLE"
        : "UNKNOWN";
  }
  return "UNKNOWN";
...
function nutrientDifference(
  ...
  for (const key of keys) {
    ...
    if (sourceValue !== null && alternativeValue !== null) {
      result[key] = Math.round((alternativeValue - sourceValue) * 10) / 10;
    }
  }
  return result;
}
...
async function loadLocalContext(userId: string) {
  ...
  return {
    ...
    consumed: {
      ...
      proteinG: sum(meals.map((meal) => meal.proteinG)),
      carbohydratesG: sum(meals.map((meal) => meal.carbsG)),
      fatG: sum(meals.map((meal) => meal.fatG)),
    },
  };
}
...
async function personalizeLocal(userId: string, nutrients: NutrientValues, food?: CanonicalFood) {
  ...
  const metrics = plan
    ? deriveNutritionPersonalization(
        nutrients,
        {
          calories: plan.dailyCalories,
          proteinG: plan.proteinGrams,
          carbohydratesG: plan.carbsGrams,
          fatG: plan.fatGrams,
  ...
  if (food?.provenance.sourceReference === "USER_CONFIRMED_PACKAGE_LABEL") {
    warnings.push(
      "Bu ürün bilgileri senin doğruladığın paket etiketinden gelir; üretici formülü değişmiş olabilir, yeni ambalajda etiketi yeniden kontrol et.",
    );
  }
...
export const nutritionPersonalizationService = {
  ...
  async compareBarcode(
    ...
    if (targetCalories === null || targetCalories <= 0) {
      throw ApiError.badRequest("Kaynak ürünün kalori verisi bulunmuyor.");
    }
    const profile = await prisma.userProfile.findUnique({
      where: { userId },
      select: { allergies: true, dietaryPreference: true },
    });
    const queries = (requestedQueries?.length ? requestedQueries : [...DEFAULT_ALTERNATIVES])
    ...
    for (const query of queries) {
      try {
        ...
        const candidate = results.find(
          (item) => !(item.provider === source.provider && item.externalId === source.externalId),
        );
        if (
          candidate &&
          !candidates.some(
        ...
        // One unavailable candidate must not invalidate the whole comparison.
      }
    }
    ...
    const comparisons = candidates
      ...
      .map((food) => {
        ...
        return {
          ...
          servingGrams: comparison.servingGrams,
          targetCalories: comparison.targetCalories,
    ...
    return {
      source: {
        food: { displayNameTr: source.displayNameTr, provider: source.provider },
        grams: sourcePortion.grams,
        nutrients: sourcePortion.nutrients,
      },
      comparisons,
      warning:
        "Aynı kalori, besinsel eşdeğerlik anlamına gelmez. Protein, karbonhidrat, yağ, lif ve şeker farklarını birlikte değerlendir.",
    };
  },
};
